...
CREATE TABLE decision (
                          ...
                          background TEXT DEFAULT NULL COMMENT '问题背景',
                          goal TEXT DEFAULT NULL COMMENT '决策目标',
                          constraints_text TEXT DEFAULT NULL COMMENT '约束条件',

                          status VARCHAR(30) NOT NULL DEFAULT 'PENDING_ANALYSIS'
                              COMMENT '决策状态：PENDING_ANALYSIS / IN_PROGRESS / PARTIAL_IN_PROGRESS / TO_CONFIRM / COMPLETED / FAILED',

                          selected_solution_id BIGINT UNSIGNED DEFAULT NULL
                              COMMENT '用户最终选择的方案ID，由业务层校验',
...
CREATE TABLE agent_step (
                            ...
                            step_type VARCHAR(30) NOT NULL DEFAULT 'THINKING'
                                COMMENT '步骤类型：THINKING / TOOL_CALL',

                            tool_name VARCHAR(100) DEFAULT NULL
                                COMMENT '工具名称，TOOL_CALL 类型时填写',

                            status VARCHAR(30) NOT NULL DEFAULT 'WAITING'
                                COMMENT '步骤状态：WAITING / RUNNING / SUCCESS / FAILED',

                            input_data JSON DEFAULT NULL
                                COMMENT '步骤输入 / 工具入参',

                            output_data JSON DEFAULT NULL
                                COMMENT '步骤结构化输出 / 工具返回结果',
...
CREATE TABLE decision_solution (
                                   ...
                                   cost_score DECIMAL(3,1) DEFAULT NULL COMMENT '成本评分',
                                   time_score DECIMAL(3,1) DEFAULT NULL COMMENT '时间评分',
                                   benefit_score DECIMAL(3,1) DEFAULT NULL COMMENT '收益评分',
                                   risk_score DECIMAL(3,1) DEFAULT NULL COMMENT '风险评分',
                                   feasibility_score DECIMAL(3,1) DEFAULT NULL COMMENT '可执行性评分',
...
CREATE TABLE decision_canvas (
                                 ...
                                 canvas_data JSON NOT NULL
                                     COMMENT '画布节点、连线及布局数据',

                                 version INT NOT NULL DEFAULT 1 COMMENT '当前画布版本',
