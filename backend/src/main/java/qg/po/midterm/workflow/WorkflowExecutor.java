...
public interface WorkflowExecutor {
     ...
     * 针对失败步骤触发重试
     *
     * @param taskId 异步推演的任�?ID
     * @param stepId 失败的步�?ID
     * @return 步骤触发重试后的状态反�?     */
    String retryStep(String taskId, String stepId);

    /**
     * 接收原始大模型输出，进行 JSON Schema 强校�?     * 如果失败，内部可触发“一次修复”逻辑
     *
     * @param jsonResult 大模型输出的 JSON 字符�?     * @return 结构化校验反馈（包含是否有效、是否修复、缺失字段等�?     */
    ValidationResult validateAndRepair(String jsonResult);

    /**
     * 底层直接运行推演节点图的方法 (�?B 同学深度集成使用)
     *
     * @param taskId 唯一任务ID，对应图�?thread_id，用�?Checkpoint 保存/恢复
